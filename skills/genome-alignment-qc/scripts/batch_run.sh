...
# ---------------------------------------------------------------------------
# batch_run.sh — genome-alignment-qc
...
Run genome-alignment-qc for multiple samples.
...
if [[ "$JOBS" -gt 1 ]] && command -v parallel &>/dev/null; then
  ...
  while IFS=$'\t' read -r sample_id bam _rest; do
    [[ -z "$sample_id" || "$sample_id" == \#* ]] && continue
    IDX=$((IDX + 1))
    outdir="${OUTDIR_PREFIX}/${sample_id}"
    echo "$sample_id"$'\t'"$bam"$'\t'"$outdir"$'\t'"$THREADS"$'\t'"$EXTRA_ARGS"$'\t'"$IDX"$'\t'"$TOTAL"$'\t'"$SCRIPT_DIR"
  done < "$SAMPLE_SHEET" | \
