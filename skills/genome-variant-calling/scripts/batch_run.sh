...
# ---------------------------------------------------------------------------
# batch_run.sh — genome-variant-calling
...
Run genome-variant-calling for multiple samples.
...
if [[ "$JOBS" -gt 1 ]] && command -v parallel &>/dev/null; then
  ...
  while IFS=$'\t' read -r sample_id bam reference _rest; do
    [[ -z "$sample_id" || "$sample_id" == \#* ]] && continue
    IDX=$((IDX + 1))
    outdir="${OUTDIR_PREFIX}/${sample_id}"
    echo "$sample_id"$'\t'"$bam"$'\t'"$reference"$'\t'"$outdir"$'\t'"$THREADS"$'\t'"$EXTRA_ARGS"$'\t'"$IDX"$'\t'"$TOTAL"$'\t'"$SCRIPT_DIR"
  done < "$SAMPLE_SHEET" | \
