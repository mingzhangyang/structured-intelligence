...
# ---------------------------------------------------------------------------
# batch_run.sh — ngs-quality-control
...
Run ngs-quality-control for multiple samples.
...
run_sample() {
  ...
  local cmd=("$script_dir/run.sh" --outdir "$outdir" --threads "$threads"
             "$fastq_r1")
  [[ -n "$fastq_r2" ]] && cmd+=("$fastq_r2")
...
if [[ "$JOBS" -gt 1 ]] && command -v parallel &>/dev/null; then
  ...
  while IFS=$'\t' read -r sample_id fastq_r1 fastq_r2 _rest; do
    ...
    cmd=("$SCRIPT_DIR/run.sh" --outdir "$outdir" --threads "$THREADS"
         "$fastq_r1")
    [[ -n "${fastq_r2:-}" ]] && cmd+=("$fastq_r2")
    [[ -n "$EXTRA_ARGS" ]] && cmd+=($EXTRA_ARGS)
