...
# ---------------------------------------------------------------------------
# batch_run.sh — genome-variant-annotation
...
Run genome-variant-annotation for multiple samples.
...
if [[ "$JOBS" -gt 1 ]] && command -v parallel &>/dev/null; then
  ...
  while IFS=$'\t' read -r sample_id vcf _rest; do
    [[ -z "$sample_id" || "$sample_id" == \#* ]] && continue
    IDX=$((IDX + 1))
    outdir="${OUTDIR_PREFIX}/${sample_id}"
    echo "$sample_id"$'\t'"$vcf"$'\t'"$outdir"$'\t'"$THREADS"$'\t'"$EXTRA_ARGS"$'\t'"$IDX"$'\t'"$TOTAL"$'\t'"$SCRIPT_DIR"
  done < "$SAMPLE_SHEET" | \
  parallel --jobs "$JOBS" --colsep $'\t' \
    run_sample {1} {2} {3} {4} {5} {6} {7} {8} \
